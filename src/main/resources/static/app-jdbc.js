// API Base URL for JDBC endpoints
...
function logTiming(operation, startTime, additionalInfo = {}) {
    ...
    console.log(`[TIMING][JDBC] ${operation}: ${duration}ms`, additionalInfo);
    return duration;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('[TIMING][JDBC] ========== Page loaded, initializing JDBC application ==========');
    loadDatasets();
...
async function loadDatasets() {
    ...
    console.log('[TIMING][JDBC] ########## Starting loadDatasets operation (JDBC) ##########');
    ...
    try {
        ...
        // Step 2: API call (to JDBC endpoint)
        const apiCallStart = Date.now();
        console.log('[TIMING][JDBC] Fetching datasets via JDBC from API...');
        const response = await fetch(`${API_BASE_URL}/datasets`);
        const apiCallTime = logTiming('Step 2/4: API call (JDBC endpoint)', apiCallStart);
        ...
        const totalTime = logTiming('########## loadDatasets (JDBC) total', operationStart, {
        ...
        console.error(`[TIMING][JDBC] Error loading datasets after ${totalTime}ms:`, error);
        hideLoading(loadingDatasets);
...
function createDatasetItem(dataset) {
    ...
    detailsDiv.textContent = `Project: ${dataset.projectId} | Method: JDBC`;
    ...
    if (createTime > 5) {
        console.log(`[TIMING][JDBC] Dataset item creation took ${createTime}ms`);
    }
...
async function loadTables(datasetId) {
    const operationStart = Date.now();
    console.log(`[TIMING][JDBC] ########## Starting loadTables for dataset: ${datasetId} ##########`);
    ...
    try {
        ...
        // Step 2: API call (to JDBC endpoint)
        const apiCallStart = Date.now();
        console.log(`[TIMING][JDBC] Fetching tables for dataset '${datasetId}' via JDBC from API...`);
        ...
        const totalTime = logTiming(`########## loadTables (JDBC) for '${datasetId}' total`, operationStart, {
            dataset: datasetId,
        ...
        const totalTime = Date.now() - operationStart;
        console.error(`[TIMING][JDBC] Error loading tables for '${datasetId}' after ${totalTime}ms:`, error);
        hideLoading(loadingTables);
...
function createTableItem(table) {
    ...
    const createTime = Date.now() - createStart;
    if (createTime > 5) {
        console.log(`[TIMING][JDBC] Table item creation took ${createTime}ms`);
    }
...
function selectTable(tableId, element) {
    const operationStart = Date.now();
    console.log(`[TIMING][JDBC] ########## Table selected: ${tableId} ##########`);
...
async function loadSchema(datasetId, tableId) {
    const operationStart = Date.now();
    console.log(`[TIMING][JDBC] ########## Starting loadSchema for table: ${datasetId}.${tableId} ##########`);
    ...
    try {
        ...
        // Step 2: API call
        const apiCallStart = Date.now();
        console.log(`[TIMING][JDBC] Fetching schema for table '${datasetId}.${tableId}' via JDBC from API...`);
        ...
        // Update stats
        schemaStats.textContent = `📊 Total Fields: ${fields.length} | Table: ${datasetId}.${tableId} | Method: JDBC`;
        ...
        const totalTime = logTiming(`########## loadSchema (JDBC) for '${datasetId}.${tableId}' total`, operationStart, {
            dataset: datasetId,
            table: tableId,
        ...
        console.log(`[TIMING][JDBC] ########## loadSchema for '${datasetId}.${tableId}' completed: ${totalTime}ms ##########`);
        
    } catch (error) {
        const totalTime = Date.now() - operationStart;
        console.error(`[TIMING][JDBC] Error loading schema for '${datasetId}.${tableId}' after ${totalTime}ms:`, error);
        hideLoading(loadingSchema);
        showError(schemaError, `Failed to load schema via JDBC: ${error.message}`);
        schemaContent.style.display = 'none';
    }
}
